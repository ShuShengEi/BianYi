...
public class Test3 {
    public static  List<SymbolsTable> symbolTable = new ArrayList<>();
    private static  List<Error> errors = new ArrayList<>();
    private static String area = "All";
    public static void main(String[] args) {
        ...
        GrammarNode node = getYi(contents);
        System.out.println(node.toString());
        removeNull();
        for (int i = 0; i<Test3.symbolTable.size();i++){
            System.out.println(Test3.symbolTable.get(i).toString());// 从根节点开始打印整棵树
        }
    }
    public static GrammarNode getYi(LexT content){//<程序>→<声明语句>main()<复合语句><函数块>
        ...
        Test3.symbolTable.add(new SymbolsTable());
        Test3.symbolTable.get(Test3.symbolTable.size() - 1).setArea(Test3.area);
        Test3.symbolTable.get(Test3.symbolTable.size() - 1).setName("Main");
        Test3.symbolTable.get(Test3.symbolTable.size() - 1).setValue("null");
        Test3.area = "main";
        num++;
        i++;
        i++;
    ...
    public static int functionDefinition(List<WordA> list, GrammarNode node, int i){//<函数定义>→<函数类型><变量>(<函数定义形参列表>)<复合语句>
        ...
        Test3.symbolTable.get(Test3.symbolTable.size() - 1).setType(list.get(i).getWord());
        Test3.symbolTable.get(Test3.symbolTable.size() - 1).setType("func" + list.get(i).getWord());
        i = functionType(list,node.getChild(num),i);
        Test3.area = list.get(i).getWord();
        if (ifContains(list.get(i).getWord())){
            Test3.symbolTable.get(Test3.symbolTable.size() - 1).setName(list.get(i).getWord() + "函数重复在第" + Integer.toString(i) + "行");
            Test3.symbolTable.get(Test3.symbolTable.size() - 1).setValue("null");
            Test3.symbolTable.get(Test3.symbolTable.size() - 1).setArea(area);
        }else {
            Test3.symbolTable.get(Test3.symbolTable.size() - 1).setName(list.get(i).getWord());
            Test3.symbolTable.get(Test3.symbolTable.size() - 1).setArea(area);
            Test3.symbolTable.get(Test3.symbolTable.size() - 1).setValue("null");
        }
    ...
    public static  int univariateDeclarations(List<WordA> list,GrammarNode node0,int i){//<单变量声明>→<变量><单变量声明0>
        ...
        node0.addChild(new GrammarNode("变量"));
        if (ifContains(list.get(i).getWord())){
            Test3.symbolTable.get(Test3.symbolTable.size() - 1).setName(list.get(i).getWord() + "变量重复声明在第" + Integer.toString(i) + "行");
            Test3.symbolTable.get(Test3.symbolTable.size() - 1).setValue("null");
        }else {
            Test3.symbolTable.get(Test3.symbolTable.size() - 1).setName(list.get(i).getWord());
            Test3.symbolTable.get(Test3.symbolTable.size() - 1).setValue("null");
    ...
    public static int constantDeclarations(List<WordA> list,GrammarNode node0,int i){//<常量声明>→const<常量类型><常量声明表>
        ...
        Test3.symbolTable.get(Test3.symbolTable.size() - 1).setType("const " + list.get(i).getWord());
...
//        i++;
    ...
    public static boolean ifContains(String a){
        for (int i = 0 ; i < Test3.symbolTable.size()  - 1 ;i++){
            try {
                if (a.equals(Test3.symbolTable.get(i).getName())){
                    if (Test3.symbolTable.get(Test3.symbolTable.size() - 1).getType().equals(Test3.symbolTable.get(i).getType()))
                        return true;
                }
            }catch (Exception e){
                return true;
    ...
    public static void removeNull(){
        for (int i = 0 ;i < Test3.symbolTable.size() - 1 ;i++){
            if (Test3.symbolTable.get(i).getName() == null){
                Test3.symbolTable.remove(i);
                i--;
            }
    ...
    public static String toAString() {
        ...
        return Test3.symbolTable.toString();
    }
}
